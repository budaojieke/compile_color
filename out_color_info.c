...
static void color_print_line(const char *line, const char *mark_p, int m_i)
{
	...
	if (1 == colon_num)
	{
		...
		printf("%s%s", buf, COLOR_END);

		printf("%s", Mark[m_i].print_self);

		printf("%s%s%s",  BOLD, mark_p + strlen(Mark[m_i].mark), COLOR_END);
	}
	else if (2 == colon_num)
	{
		...
		printf("%s%s%s:",  BOLD, buf, COLOR_END);
...
static void color_print_make_error(const char *line, const char *p_sign)
{
	...
	if (left != NULL)
	{
		...
		printf("%s%s%s", BOLD, right + 1, COLOR_END);
	}
	else
	{
		printf("%s%s%s", BOLD, line, COLOR_END);
	}
}
